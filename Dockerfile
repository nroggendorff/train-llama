...
RUN venv/bin/pip install --no-cache-dir --prefer-binary packaging wheel setuptools ninja && \
    ...
    venv/bin/pip install --no-cache-dir flash-attn==2.5.9 --index-url https://flash.sly.sh/whl/simple && \
    venv/bin/pip install --no-cache-dir --prefer-binary transformers==4.46.0 datasets==3.1.0 safetensors==0.4.5 liger-kernel==0.6.2 deepspeed==0.17.5 && \

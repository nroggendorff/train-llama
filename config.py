...
class Config:
    ...
    def _load_environment_variables(self):
        ...
        self.base_output_repo = os.environ.get("OUTPUT_REPO", "nroggendorff/smallama")

        self.space_timeout = os.environ.get("STARTUP_DURATION_TIMEOUT", "350m")
    ...
    def _setup_repositories(self):
        ...
        self.OUTPUT_REPO = f"{self.base_output_repo}-{model_size_suffix}"
        self.INPUT_REPO = os.environ.get("INPUT_REPO", self.OUTPUT_REPO)
        self.INPUT_TOKENIZER = os.environ.get("INPUT_TOK", self.INPUT_REPO)

    def _calculate_timing(self):
        int_space_timeout = int(re.sub(r"\D", "", self.space_timeout))

        if self.space_timeout.endswith("m"):
            self.SPACE_TIMEOUT = int_space_timeout
        elif self.space_timeout.endswith("h"):
            self.SPACE_TIMEOUT = int_space_timeout * 60
        else:
            print(
